...
pub fn validate(parser: &ProtoParser) -> Result<(), ValidatorError> {
    ...
    parser.message_types.iter().for_each(|(id, _)| {valid_message_types.insert(id);});
    parser.enum_types.iter().for_each(|(id, _)| {valid_message_types.insert(id);});
                ...
                FieldType::MessageType(identifier, _)  | FieldType::UnboundedMessageType(identifier) 
                 => {
                    if !valid_message_types.contains(identifier) {
                        return Err(ValidatorError::MissingTypeDefinition(identifier.to_string()));
                    }
                }
                _ => {},
            }

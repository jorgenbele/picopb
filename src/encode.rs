...
impl EncodeBuffer<'_> {
    ...
    pub fn encode_sub_message(&mut self, encodable: impl ToWire, field: Field) -> std::io::Result<usize> {
        encodable.append(self)
    }
...
pub fn precalculate_prefix_size(len: usize) -> usize {
    let (_, count) = (len as u64).to_varint_encoding();
    count
}
